 ...
 * ingest-opc.ts
 *
 * Downloads the Canada OPC (Office of the Privacy Commissioner) list of
 * organizations investigated or known to handle personal data commercially,
 * and merges data broker entries into data/brokers.yaml.
 *
 * Usage: npx tsx scripts/ingest-opc.ts [--dry-run] [--file <path>]
 *
...
const OPC_URL =
  "https://www.priv.gc.ca/en/report-a-concern/file-a-formal-privacy-complaint/data-broker-registry.csv";
...
async function main(): Promise<void> {
  ...
  if (localFile) {
    ...
    console.log(`\nReading OPC data from local file:\n  ${localFile}`);
    csvText = readFileSync(localFile, "utf-8");
  } else {
    console.log(`\nDownloading OPC register from:\n  ${OPC_URL}`);
    const response = await fetch(OPC_URL);
    if (!response.ok) {
      throw new Error(`Failed to download OPC register: ${response.status} ${response.statusText}`);
    }
  ...
  console.log(`OPC entities (with website): ${dataRows.length}`);
  ...
  console.log(`  (${existing.brokers.length} existing + ${newBrokers.length} new from OPC)`);
}
