...
describe('crypto', () => {
  it('round-trips plaintext through encrypt/decrypt', async () => {
    const key = await deriveKey('test-passphrase', new Uint8Array(32).fill(1))
  ...
  it('deriveKey produces different keys for different salts', async () => {
    const key1 = await deriveKey('same', new Uint8Array(32).fill(1))
    const key2 = await deriveKey('same', new Uint8Array(32).fill(2))
